...
const Legend = ({ onZoomIn, onZoomOut, onPan, onReset, zoom }) => {
  const Btn = ({ children, onClick, wide }) => (
    <button onClick={onClick} style={{ width: wide ? 60 : 28, height: 28, borderRadius: 5, border: `1px solid ${C.border}`, background: C.surfaceLight, color: C.textDim, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontFamily: "monospace", padding: 0, lineHeight: 1, transition: "all 0.15s" }}
      onMouseEnter={e => { e.currentTarget.style.background = C.border; e.currentTarget.style.color = C.text; }}
      onMouseLeave={e => { e.currentTarget.style.background = C.surfaceLight; e.currentTarget.style.color = C.textDim; }}
    >{children}</button>
  );
  return (
    <div style={{ position: "absolute", top: 10, right: 10, display: "flex", flexDirection: "column", gap: 8, zIndex: 10 }}>
      {/* Legend */}
      <div style={{ background: C.surfaceLight, border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 12px", fontSize: 9, color: C.textDim, display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      {/* Zoom & Pan Controls */}
      <div style={{ background: C.surfaceLight, border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 10px", display: "flex", flexDirection: "column", gap: 6, alignItems: "center" }}>
        <div style={{ fontSize: 8, color: C.textMuted, fontFamily: "monospace", textTransform: "uppercase", letterSpacing: "0.06em" }}>Zoom & Pan</div>
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
          <Btn onClick={onZoomOut}>{"\u2212"}</Btn>
          <div style={{ width: 44, textAlign: "center", fontSize: 10, color: C.text, fontFamily: "monospace", fontWeight: 600 }}>{Math.round(zoom * 100)}%</div>
          <Btn onClick={onZoomIn}>+</Btn>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "28px 28px 28px", gridTemplateRows: "28px 28px 28px", gap: 3 }}>
          <div />
          <Btn onClick={() => onPan(0, 1)}>{"\u25B2"}</Btn>
          <div />
          <Btn onClick={() => onPan(1, 0)}>{"\u25C0"}</Btn>
        ...
        <Btn onClick={onReset} wide>Reset</Btn>
      </div>
    </div>
  );
};
...
export default function RINADiagram() {
  ...
  const [pan, setPan] = useState({ x: 0, y: 0 }); // in SVG world coords
  const [zoom, setZoom] = useState(1);
  const [dragging, setDragging] = useState(false);
  const svgRef = useRef(null);
  const isPanning = useRef(false);
  const lastMouse = useRef({ x: 0, y: 0 });
  const show = useCallback((key) => { if (componentInfo[key]) setInfo(prev => prev && prev.title === componentInfo[key].title ? null : componentInfo[key]); }, []);

  // Parse base viewBox for the current view
  const baseVb = useCallback(() => {
    const vbs = { overview: "0 0 1020 670", dataflow: "0 0 1060 720", efcp: "0 0 1020 660", allocation: "0 0 1020 680" };
    return vbs[view].split(" ").map(Number);
  }, [view]);

  // Compute dynamic viewBox: zoom shrinks the visible area, pan shifts it
  ...
  const handleWheel = useCallback((e) => {
    e.preventDefault();
    setZoom(z => Math.max(0.25, Math.min(4, z * (e.deltaY > 0 ? 0.92 : 1.08))));
  }, []);

  const handleMouseDown = useCallback((e) => {
    if (e.button === 0) { isPanning.current = true; setDragging(true); lastMouse.current = { x: e.clientX, y: e.clientY }; }
  }, []);
  ...
  const handleMouseMove = useCallback((e) => {
    if (isPanning.current && svgRef.current) {
      ...
      const [, , bw, bh] = baseVb();
      // Convert screen pixel delta to SVG world coordinate delta
      const dx = (e.clientX - lastMouse.current.x) * (bw / zoom) / rect.width;
      const dy = (e.clientY - lastMouse.current.y) * (bh / zoom) / rect.height;
      setPan(p => ({ x: p.x + dx, y: p.y + dy }));
      lastMouse.current = { x: e.clientX, y: e.clientY };
    }
  }, [baseVb, zoom]);

  const handleMouseUp = useCallback(() => { isPanning.current = false; setDragging(false); }, []);
  ...
  const zoomOut = () => setZoom(z => Math.max(0.25, z * 0.8));
  const panStep = (dx, dy) => { const step = 50 / zoom; setPan(p => ({ x: p.x + dx * step, y: p.y + dy * step })); };
  ...
  const views = { overview: { render: renderOverview }, dataflow: { render: renderDataFlow }, efcp: { render: renderEFCPDetail }, allocation: { render: renderFlowAllocation } };
  ...
  return (
    <div style={{ width: "100%", height: "100vh", background: C.bg, display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
        <Legend onZoomIn={zoomIn} onZoomOut={zoomOut} onPan={panStep} onReset={resetView} zoom={zoom} />
        <svg ref={svgRef} width="100%" height="100%" viewBox={getViewBox()} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} style={{ cursor: dragging ? "grabbing" : "grab" }}>
          ...
          <rect x="-9999" y="-9999" width="99999" height="99999" fill={C.bg} /><rect x="-9999" y="-9999" width="99999" height="99999" fill="url(#grid)" />
          {cur.render()}
        </svg>
